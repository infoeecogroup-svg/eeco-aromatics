...
export default function HiddenLinksPage() {
  ...
  // Dynamic Page Title
  useEffect(() => {
    document.title = `${businessProfile?.businessName || 'EECO AROMATICS'} | Official Business & Social Links`;
  ...
  const renderLinkIcon = (icon: string) => {
    switch (icon.toLowerCase()) {
      ...
      case 'store':
        return (
          <div className="linktree-icon-bubble" style={{ background: 'linear-gradient(135deg, #1A56DB, #00A699)' }}>
      ...
      case 'facebook':
        return (
          <div className="linktree-icon-bubble fb-bubble">
            <svg width="19" height="19" fill="#FFFFFF" viewBox="0 0 24 24">
      ...
      case 'instagram':
        return (
          <div className="linktree-icon-bubble insta-bubble">
            <svg width="19" height="19" fill="currentColor" viewBox="0 0 24 24" color="#FFFFFF">
  ...
  return (
    <main className="linktree-main-container">
      ...
      <div className="linktree-card-wrapper">
        {/* Top Action Bar (Copy / Share Link) */}
        <div className="linktree-top-actions">
          <button
            onClick={handleShare}
          ...
          <button
            ...
            className={`linktree-copy-btn ${copied ? 'copied' : ''}`}
        ...
        >
          <div className="linktree-logo-wrapper">
            <img
              src={businessProfile?.logo || '/eeco_logo.png'}
              alt={businessProfile?.businessName || 'Business Logo'}
              className="linktree-logo-img"
            />
            <div className="linktree-logo-ring"></div>
          </div>
          ...
          <p className="linktree-description">
          ...
          {businessProfile?.location && (
            <div className="linktree-location-pill">
        ...
        {/* Links Stack */}
        <div className="linktree-links-stack">
          {activeLinks.map((link, index) => {
            ...
            return isInternal ? (
              <Link key={link.id || index} href={link.url} className="linktree-btn-link">
                {content}
              </Link>
            ) : (
              <a
        ...
        <div className="linktree-guarantee-box">
          ...
          <span>100% Genuine Sri Lankan Herbal Formulations • Fast Islandwide Delivery</span>
        </div>
